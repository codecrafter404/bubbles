...
type OrderQuery struct {
	...
	withCustomItems      *OrderCustomItemQuery
	modifiers            []func(*sql.Selector)
	loadTotal            []func(context.Context, []*Order) error
	withNamedItems       map[string]*OrderItemQuery
	withNamedCustomItems map[string]*OrderCustomItemQuery
...
// QueryCustomItems chains the current query on the "custom_items" edge.
func (_q *OrderQuery) QueryCustomItems() *OrderCustomItemQuery {
...
func (_q *OrderQuery) Clone() *OrderQuery {
	...
	return &OrderQuery{
		...
		withCustomItems: _q.withCustomItems.Clone(),
...
// WithCustomItems tells the query-builder to eager-load the nodes that are connected to
// the "custom_items" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *OrderQuery) WithCustomItems(opts ...func(*OrderCustomItemQuery)) *OrderQuery {
	query := (&OrderCustomItemClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withCustomItems = query
	return _q
}
...
func (_q *OrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Order, error) {
	...
	if query := _q.withCustomItems; query != nil {
		if err := _q.loadCustomItems(ctx, query, nodes,
			func(n *Order) { n.Edges.CustomItems = []*OrderCustomItem{} },
			func(n *Order, e *OrderCustomItem) { n.Edges.CustomItems = append(n.Edges.CustomItems, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range _q.withNamedItems {
		if err := _q.loadItems(ctx, query, nodes,
			func(n *Order) { n.appendNamedItems(name) },
			func(n *Order, e *OrderItem) { n.appendNamedItems(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range _q.withNamedCustomItems {
		if err := _q.loadCustomItems(ctx, query, nodes,
			func(n *Order) { n.appendNamedCustomItems(name) },
			func(n *Order, e *OrderCustomItem) { n.appendNamedCustomItems(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *OrderQuery) loadCustomItems(ctx context.Context, query *OrderCustomItemQuery, nodes []*Order, init func(*Order), assign func(*Order, *OrderCustomItem)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Order)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.OrderCustomItem(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(order.CustomItemsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.order_custom_items
		if fk == nil {
			return fmt.Errorf(`foreign-key "order_custom_items" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "order_custom_items" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *OrderQuery) WithNamedItems(name string, opts ...func(*OrderItemQuery)) *OrderQuery {
	query := (&OrderItemClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if _q.withNamedItems == nil {
		_q.withNamedItems = make(map[string]*OrderItemQuery)
	}
	_q.withNamedItems[name] = query
	return _q
}

// WithNamedCustomItems tells the query-builder to eager-load the nodes that are connected to the "custom_items"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (_q *OrderQuery) WithNamedCustomItems(name string, opts ...func(*OrderCustomItemQuery)) *OrderQuery {
	query := (&OrderCustomItemClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if _q.withNamedCustomItems == nil {
		_q.withNamedCustomItems = make(map[string]*OrderCustomItemQuery)
	}
	_q.withNamedCustomItems[name] = query
	return _q
}
